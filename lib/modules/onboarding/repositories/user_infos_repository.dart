import 'package:fitnect/modules/onboarding/api/user_infos_api.dart';
...
final userInfosRepositoryProvider = Provider<UserInfosRepository>(
  (ref) => UserInfosRepository(userInfosApi: ref.read(userInfosApiProvider)),
);

class UserInfosRepository {
  final UserInfosApi _userInfosApi;

  UserInfosRepository({required UserInfosApi userInfosApi})
    : _userInfosApi = userInfosApi;

  Future<void> save(String userId, UserInfoDetail info) async {
    final entity = info.toEntity(userId);
    final alreadyExistingInfo = await _userInfosApi.getByKey(
      userId,
      entity.key,
    );
